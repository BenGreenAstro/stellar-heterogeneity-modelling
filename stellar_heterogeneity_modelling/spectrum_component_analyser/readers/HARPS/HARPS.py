...
def read_HARPS_fits(fits_absolute_path : Path, verbose : bool = False, name : str = None, INTEGRATION_INDEX : int = 0) -> spectrum:
	...
	with fits.open(fits_absolute_path) as hdul:
		...
		spec : spectrum = spectrum(
				  wavelengths = data["WAVE"][INTEGRATION_INDEX] * HARPS_WAVELENGTH_UNITS,
				  fluxes = data["FLUX"][INTEGRATION_INDEX] * HARPS_FLUX_UNITS,
				  name=name,
				  normalised_point=HARPS_normalising_point,
				  normalise_flux=True
				  )
